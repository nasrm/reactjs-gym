    ...
    return (
        <SectionWrapper id={'generate'} header={"generate your workout"} title={["It\'s", "Huge", "o\'clock"]}>
            ...
            <div className='bg-slate-950 border border-solid border-blue-400 rounded-lg flex flex-col'>
                <button onClick={toggleModel} className='relative flex items-center py-3 justify-center'>
                ...
                {showModal && (
                    <div className='flex flex-col px-3 pb-3'>
                        {(poison === 'individual' ? WORKOUTS[poison] : Object.keys(WORKOUTS[poison])).map((muscleGroup, muscleGroupIndex) => {
                            return (
                                <button onClick={() => updateMuscles(muscleGroup)} className={'hover:text-blue-400 '+ (muscles.includes(muscleGroup) ? 'text-blue-400': '')} key={muscleGroupIndex}>
            ...
            <div className='grid grid-cols-1 sm:grid-cols-3 gap-4'>
                {Object.keys(SCHEMES).map((scheme, schemeIndex) => {
                    return (
                        <button onClick={() => {
                            setGoal(scheme)
                        }} className={'bg-slate-950 border border-blue-400 duration-200 hover:border-blue-600 px-4 py-3 rounded-lg ' + (scheme === goal ? 'border-blue-600' : 'border-blue-400')} key={schemeIndex}>
